...
def compare_scores_vs(xvar, alg_order=alg_order, metric='MCC'):
    ...
    g.add_legend(loc=(0.35,0.15), ncols=3)
    return g
...
```{python}
compare_scores_vs('edge-density', metric='F-M')
```
...
df_improv = pd.DataFrame([
    tidy.pivot(index=['ID','metric'], columns='name', values='score').pipe(lambda df: df['FP']>df['GL']).rename('score-better'),
    tidy.pivot(index=['ID','metric'], columns='name', values='seconds').pipe(lambda df: df['FP']<df['GL']).rename('time-better')
...
(so.Plot(tidy, x='n-nodes',y='seconds',color='name')
 ...
 .scale(
     ...
     y='log',
     # marker=so.Nominal(["_", "."]),
     color=so.Nominal('Set2',order=['FP', 'GL', 'HSS'])
 )
 .theme(theme_config)
 .on(sf1)
 .plot()
...
p = (so.Plot(tidy, y='seconds',color='name',marker='failed', pointsize='failed')
 .pair(x=['edge-density','med-actv-ct'])
...
feats = ['n-walks','med-actv-ct', 'n-nodes']
