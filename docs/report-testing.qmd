...
# BA.shape
# _estimators['TS'](X)
from affinis.associations import _pursue_tree_basis, _spanning_forests_obs_bootstrap, SFD_edge_prob
from affinis.utils import groupby_col0
...
def compare_for_dataset(dataset_id, legend=True):
    ...
    def handle_algs(algs,sf):
        for est in algs:
            ...
            M = m.Contingent.from_scalar(gT,gP)
            print(est,':\t',f'{mcc_f1(M):.3f}')
            # sf.plot(M.weights, M.mcc, label=est)
            # sf.scatter(M.F, (M.mcc+1)/2, label=est, marker='.')
            sf.plot(np.ma.log10(M.y_pred.sum(axis=1)/M.y_true.sum()), M.mcc, label=est)
        # sf1.legend(**legend_kws)
    
    handle_algs(['FP','FPi', 'GL'], sf1)
...
```{python}
compare_for_dataset(list(_datasets)[200], legend=False)
```
